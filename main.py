...
def main():
    ...
    parser.add_argument('imageset_ids', type=int, nargs='+', help='the ids of the imagesets that are downloaded')

    args = parser.parse_args()

    imageset_ids = args.imageset_ids
    destination = args.destination if args.destination else '.'
    if args.destination and not os.path.exists(destination):
    ...
    if protobuf_labels and no_images:
        print("Invalid options, -n and -p are not compatible")
        sys.exit()
    ...
    for imageset_id in imageset_ids:
        print("Downloading imageset: " + str(imageset_id))
